    ...
    <div class="page-container">
      ...
      <main class="main-content">
        <div class="preview-header">
          <h2 class="preview-title">英文单词字帖</h2>
        ...
        <div class="preview-wrapper">
          ...
          >
            <!-- 多页显示 -->
            <div 
              v-for="(pageWords, pageIndex) in generatedPages"
              :key="pageIndex"
              ref="worksheetRef"
              class="worksheet-paper"
            >
              <div 
                v-for="(word, index) in pageWords" 
                :key="`${pageIndex}-${index}`"
                class="word-cell"
                :class="{
                  'with-lines': settings.showLines,
                  'is-trace': word && shouldTrace(index),
                  'is-empty': !word
                }"
                :style="cellStyle"
              >
      ...
      <aside class="sidebar">
        <div class="sidebar-content">
          ...
          <section class="control-section">
            <h3 class="section-title">输入单词</h3>
            <button @click="showInputDialog = true" class="btn btn-primary">
              <span class="btn-icon">✏️</span>
              输入内容
            </button>
            <div class="input-hint">提示：点击按钮输入要练习的单词，每个单词占一行</div>
          </section>
          ...
          <section class="control-section">
            ...
            <div class="form-row">
              <label class="form-label">每行单词数</label>
...
const cellStyle = computed(() => {
  ...
  return {
    ...
    fontSize: `${Math.floor(cellSize * 0.3 * settings.fontSizeRatio)}px`,
    fontFamily: settings.fontFamily,
    color: '#000000'
  }
})
...
const rowsPerPage = computed(() => {
  ...
  const cellHeight = Math.floor(555 / settings.columns * 0.6)
  return Math.ceil(availableHeight / cellHeight)
})
...
const generatedPages = computed(() => {
  ...
  lines.forEach(line => {
    ...
    // 如果当前页已满，添加到页面数组
    if (currentPage.length + wordRow.length > settings.columns * rowsPerPage.value) {
      pages.push(currentPage)
      currentPage = []
    }
    
    currentPage.push(...wordRow)
  })
...
/* 四线格样式 */
.word-cell.with-lines::before,
.word-cell.with-lines::after,
.word-cell.with-lines span.line-top,
.word-cell.with-lines span.line-middle {
  content: '';
  position: absolute;
