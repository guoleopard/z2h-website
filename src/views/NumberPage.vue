<template>
  <div class="number-worksheet-container">
    <div class="page-container">
      <!-- 左侧预览区域 -->
      <main class="main-content">
        <div class="preview-header">
          <h2 class="preview-title">数字字帖预览</h2>
        ...
        <div class="preview-wrapper">
          ...
          >
            ...
            >
              <div 
                v-for="(number, index) in pageChars" 
                :key="`${pageIndex}-${index}`"
                class="number-cell"
                :class="{
                  'with-grid': settings.showGrid,
                  'with-cross': settings.showCross,
                  'is-trace': number && shouldTrace(index),
                  'is-empty': !number
                }"
                :style="cellStyle"
              >
      ...
      <aside class="sidebar">
        <div class="sidebar-content">
          ...
          <section class="control-section">
            <h3 class="section-title">输入数字</h3>
            <button @click="showInputDialog = true" class="btn btn-primary">
              <span class="btn-icon">✏️</span>
              输入内容
            </button>
            <div class="input-hint">提示：点击按钮输入要练习的数字，支持换行</div>
          </section>
            ...
            <div class="dialog-content" @click.stop>
              ...
              <div class="dialog-body">
                <textarea 
                  ...
                  placeholder="请输入要练习的数字，例如'0123456789'，支持插入空行。"
...
// 输入文字
const inputText = ref('0123456789')
...
const settings = reactive({
  ...
  traceColor: '#cccccc',
  fontSizeRatio: 0.9 // 字体大小比例，占田字格高度的90%
})
...
const generatedPages = computed(() => {
  ...
  lines.forEach(line => {
    ...
    chars.forEach(char => {
      ...
      // 补充空格填满这一行
      const emptyCount = settings.columns - (2 % settings.columns)
...
/* 最后一列的单元格显示右边框 */
.number-cell:nth-child(12n) {
  border-right: 0.2px solid #e0e0e0;
}

/* 田字格样式 */
.number-cell.with-grid {
  background-image: 
...
/* 十字线样式 */
.number-cell.with-cross::before,
.number-cell.with-cross::after {
  content: '';
  position: absolute;
...
.number-text {
  ...
  transform: translateX(-50%) skew(-5deg); /* 左移50%并倾斜5度 */
  width: 50%;
  text-align: center;
  letter-spacing: 2px; /* 增加字符间距，拉长字体 */
}

/* 描红样式 */
.number-cell.is-trace .number-text {
  -webkit-text-stroke: 1px #ff4444;
  text-stroke: 1px #ff4444;
